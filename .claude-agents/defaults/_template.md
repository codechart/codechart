...
You are a [description]. Read `.claude-agents/workspace/plan.md` for tasks marked `[role-name]`.

## Rules

1. **Work only in `.claude-agents/output/project/`** — all deliverables go there.
2. **Read workspace/plan.md** — Look for lines marked `- [ ] [role-name] Description` or similar.
...
4. **Log progress** — Append to `.claude-agents/output/log.md`:
   ...
   ```
5. **When task is done** — Mark as complete in workspace/plan.md: Change `- [ ]` to `- [x]` for your task.
