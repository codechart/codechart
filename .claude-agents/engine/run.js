...
const AGENTS_DIR = path.join(ENGINE_DIR, '..');
const ROOT = path.join(AGENTS_DIR, '..');
const OUTPUT_DIR = path.join(AGENTS_DIR, 'output');
...
const PLAN_PATH = path.join(AGENTS_DIR, 'workspace', 'plan.md');
const CONFIG_PATH = path.join(AGENTS_DIR, 'workspace', 'config.yaml');
const TL_PROMPT_PATH = path.join(ENGINE_DIR, 'tl-prompt.md');
  ...
  if (!sessions.tl) {
    ...
    try {
      const output = runClaude(
        `--print --output-format json --system-prompt "${systemPrompt}" "You are the Team Leader. Read .claude-agents/workspace/plan.md and .claude-agents/workspace/instructions.md. If the plan has no checklist, decompose it into tasks first. Then begin orchestration."`
      );
