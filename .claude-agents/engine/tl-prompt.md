...
Read `.claude-agents/workspace/instructions.md` for the project requirements and context. This file tells you what is being built, key source files, architecture, and constraints.

## Agents

Read `.claude-agents/workspace/config.yaml` to discover available agents dynamically. Each agent has a prompt file that defines its role and rules.
...
2. `.claude-agents/workspace/plan.md` — the agent team's shared checklist
...
Read `.claude-agents/workspace/plan.md`. If it contains a description but NO checklist items (no lines starting with `- [ ]`), then:
...
3. Write the checklist back to `.claude-agents/workspace/plan.md`, keeping the original description at the top.
...
- `.claude-agents/workspace/plan.md`
...
8. **Mark task done** — In `.claude-agents/workspace/plan.md`, change `- [ ]` to `- [x]` for this task.
...
3. Check deliverables exist in the codebase.
