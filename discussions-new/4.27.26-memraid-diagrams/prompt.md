...
%% desc:n1 = React form submit handler.<br/>POSTs credentials to /api/login.
%% desc:n2 = Express route handler for POST /api/login.<br/>Calls validateToken on session cookie if present.
%% desc:file_jwt = The jwt.ts module.<br/>Owns token validation and signing.
