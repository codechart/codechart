...
### general
we want to follow high level logc, with branches for lower level logic, inner logic, declerations and configurations
...
- actions that are linked to type declerations are connected to next action, and to the type decleration
...
### example code
see [example code](./example-code.ts)
### example output
