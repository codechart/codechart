...
function clickedOnLine(ideEventObject, lineContent, lineNumber, filePath, projectPath, fileContent) {
    frameElement.contentWindow.postMessage({action: 'clickedOnLine',data: {
...
function clickedOnFile(ideEventObject, fileOrFolderPath, projectPath, fileContent, filesInFolder) {
    frameElement.contentWindow.postMessage({action: 'clickedOnFile',data: {
...
function displayInputInReadmeElement(readmeText) {
    frameElement.contentWindow.postMessage({action: 'displayContentInReadmeElement',data: {
