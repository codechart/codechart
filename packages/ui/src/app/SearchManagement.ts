...
export class SearchManagement {
  ...
  setPaths(paths: CCPath[], selectedPath: string) {
    ...
    paths.sort((i, j) => {
      if (i.folder === storedPath) return -1
      else return 0
    })
    this._projectPaths = paths
    this.app.dropdownPaths = paths.map((i) => {
      return { label: i.label, value: i.folder }
    })
    if (!selectedPath) {
      this.setSelectedPath(this._projectPaths[0])
    } else {
      this.setSelectedPath(
        this._projectPaths.find((i) => i.folder === selectedPath)
      )
    }

    if (paths.find((i) => !i.gitUrl) && !this.ideConnect.getIsInIde())
      this.app.addMessage(
        'Some project folders are not git repos',
        'Some of the project folders are not aligned with git repos. To align your folders use the edit nutton next to the project drow-down',
        -1
      )
  }
  ...
  setSelectedPath(path: CCPath) {
    ...
    let convertPathToObject = (
      items: string[],
      index,
      currentLeaf: { id; label; data; children }[],
      id
    ) => {
      ...
      if (childIndex === -1) {
        if (finalItem) {
          myChildren = Object.assign(
            { id: id, label: myName, data: myName },
            { icon: 'fa-file-code-o' }
          )
    ...
    let convertPathArrayToObject = (paths: string[], object) => {
      this.splitChar =
        this.searchObject.folderPath.folder.indexOf('/') == -1 ? '\\' : '/'
      for (const path of paths) {
        let lastId = 0
        lastId = convertPathToObject(
          path.split(this.splitChar),
          0,
          object,
          lastId
        )
    ...
    this.http
      .post(Env.getApiEndpoint() + EndPoints.getAllFilesInPath, path)
      .subscribe((res: { files: string[] }) => {
        this.app.availableFiles = res.files.map((i) => {
          return {
            fullPath: i,
            fromSource: i.substring(
              this.searchObject.folderPath.folder.length,
              i.length
            ),
          }
        })
        ...
        try {
          convertPathArrayToObject(
            this.app.availableFiles.map((i) => i.fromSource),
            this.app.fileTreeNodes
          )
          this.app.fileTreeNodes = this.app.fileTreeNodes.sort((i, j) =>
            !i.children ? 1 : -1
          )
  ...
  public getPathByGitUrl(gitUrl: string) {
    return this.projectPaths.find(
      (projectPath) => projectPath.gitUrl === gitUrl
    )
  }
  ...
  setProjectPath(path, index): Promise<CCPath[]> {
    return new Promise((resolve, reject) => {
      ...
      if (index === -1) {
        this.http
          .post(Env.getApiEndpoint() + EndPoints.addPath, {
            path: path,
          })
          .toPromise()
          .then((res: CCPath) => {
            ...
            if (this.app.addFileInput)
              this.app.addFileInput.nativeElement.value = ''
            resolve(res)
          })
          .catch((ex) => {
            onFail(ex)
        ...
        this.http
          .post(Env.getApiEndpoint() + EndPoints.setPaths, {
