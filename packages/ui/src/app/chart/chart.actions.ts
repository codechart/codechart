...
export enum PositioningOptions {
  DOWN,
  RIGHT,
  LEFT,
  UP,
...
export class ChartActions {
  ...
  getMatchNodesPositions(matchNodes: Node[], alignToPos: { x; y }): { x; y }[] {
    ...
    let firstInRangePos
    if (
      this.app.Options.positioning === PositioningOptions.LEFT ||
      this.app.Options.positioning === PositioningOptions.RIGHT
    ) {
  ...
  private positionNextToOverlappingNodes(
    positions: { x; y }[],
    matchPos: { x; y },
    checkNodes: IdType[]
  ): { x; y }[] {
    ...
    const allMatchIdsOfSamePos = this.chart
      .getItems(checkNodes)
      .nodes.filter(
        (i) =>
          i[fixedPosKey] >= fixedPosToCheck - ChartConsts.gridBaseSize &&
          i[fixedPosKey] <= fixedPosToCheck + ChartConsts.gridBaseSize
      )
    if (allMatchIdsOfSamePos.length > 0) {
      const largestVaryingMatchPos = allMatchIdsOfSamePos
        .map((i) => i[varyingPosKey])
        .sort((a, b) => {
          return b - a
        })[0]
      ...
      correctedPositions = sortedPositions.map((i, index) => {
        ...
        newPos[varyingPosKey] =
          largestVaryingMatchPos + MatchDistance.betweenMatches() * (index + 1)
  ...
  public positionNormal(addedItems: Array<Node | Edge>) {
    ...
    let alignToPos = this.app.selectedNode
      ? this.chart.getPosition(this.app.selectedNode.id)
      : { x: 0, y: 0 }
    let matchNodes: Node[] = addedItems.filter((i) =>
      ChartUtils.isMatchNode(i as Node)
    ) as Node[]
    let positions: { x; y }[] = []
    ...
    if (this.app.selectedNode) {
      possibleOverlapNodes = this.chart.getNeighboursByEdge(
        this.app.selectedNode.id,
        (edge: Edge) => true
      ).nodes
      possibleOverlapNodes = this.chart
        .getItems(possibleOverlapNodes)
        .nodes.filter((i) => ChartUtils.isMatchNode(i))
        .map((i) => i.id)
    } else
      possibleOverlapNodes = this.chart
        .getAllMatchNodes()
    ...
    positions = this.positionNextToOverlappingNodes(
      positions,
      alignToPos,
      possibleOverlapNodes
    )
    matchNodes = matchNodes.map((i, index) => {
      i.x = positions[index].x
      i.y = positions[index].y
    ...
    matchNodes.forEach((matchNode: MatchNode) => {
      ...
      if (!ChartUtils.isCustomNode(ofFileNode)) {
        let ofFileItems = CreateUtils.createFileNameNode(
          ofFileNode.label,
          matchNode,
          ofFileNode.color as any,
          this.chart
        )
  ...
  public addToChartAndPosition(
    nodesAndLinks: Array<Node | Edge>,
    options: { moveBelowExisting } = { moveBelowExisting: true }
  ): Array<Node | Edge> {
    try {
      let addedIds = nodesAndLinks
        .filter((i) => {
      ...
      let newNodesAndLinks = nodesAndLinks
        .map((item) => {
          ...
          if (itemOnChart !== null) {
            ...
            ChartUtils.setAttributes(
              item as Node,
              ChartUtils.getMatchAttributes(item as Node)
            )
      ...
      updatedNodes.forEach((i) => {
        if (
          ChartUtils.isFileNode(i) &&
          this.chart.getNeighbours(i.id).nodes.length === 0
        )
          i.hidden = false
      })
      ...
      console.log(
        newNodesAndLinks
          .filter((i: Node) => ChartUtils.isMatchNode(i))
          .map((i: Node) => i.y)
      )
      ...
      setTimeout(() => {
        ...
        this.setInnerContentEdges(
          ...
          CcItemStyles.insideContentLink,
          ContentEdgeTypes.insideContent,
          addedMatches as Node[],
          currentMatches
        )
  ...
  private setInnerContentEdges(
    getOtherEndLine: (node: Node) => number,
    edgeStyle: any,
    edgeType: ContentEdgeTypes_type,
    addedMatches: Node[],
    existingMatches: Node[]
  ) {
    let addedEdges: Edge[] = []
    addedMatches.forEach((i) => {
      let contentLinks: {
        otherEndLineNumber: number
      ...
      existingMatches.forEach((j) => {
        ...
        if (
          (myEndLineNumber &&
            isInside(myEndLineNumber, otherLineNumber, otherEndLineNumber)) ||
          isInside(myLineNumber, otherLineNumber, otherEndLineNumber)
        ) {
          contentLinks.push({
            otherEndLineNumber: otherEndLineNumber,
            otherLineNumber: otherLineNumber,
            edge: this.chart.createLink(j.id, i.id, edgeStyle, {
              idPrefix: edgeType,
            }),
          })
        } else if (
          (otherEndLineNumber &&
            isInside(otherEndLineNumber, myLineNumber, myEndLineNumber)) ||
          isInside(otherLineNumber, myLineNumber, myEndLineNumber)
        ) {
          contentLinks.push({
            otherEndLineNumber: otherEndLineNumber,
            otherLineNumber: otherLineNumber,
            edge: this.chart.createLink(i.id, j.id, edgeStyle, {
              idPrefix: edgeType,
            }),
          })
        }
      ...
      if (contentLinks.length > 0) {
        if (contentLinks.length > 1)
          contentLinks.sort(
            (i, j) =>
              i.otherEndLineNumber -
              i.otherLineNumber -
              (j.otherEndLineNumber - j.otherLineNumber)
          )
  ...
  private setFileNodePos_Directional(
    node: Node,
    fileNodeIndex: number,
    largestYPos
  ) {
    if (this.chart.getItem(node.id) && this.chart.getItem(node.id) !== null)
      return node
    let xPos, yPos
    ...
    if (selectedNodes !== null && selectedNodes.length > 0) {
      ...
      if (shapeInfo.details.createLinkToFile && selectedNodes.length === 1) {
        ...
        if (ChartUtils.isOfFile(node) || ChartUtils.isFileNode(node)) {
          ...
          let fileLink = CreateUtils.createFileEdge(
            this.chart,
            fileNode,
            newNode.id
          )
  ...
  public positionAndLinkToSelected(
    newNode: Node,
    addedItems: (Node | Edge)[],
    linkInfo: EdgeOptions
  ) {
    let selectedNodes = this.chart
      .getSelection()
      .nodes.map((i) => this.chart.getNode(i)) as Node[]
    let xPos = 0
    let yPos = 0
    ...
    if (selectedNodes.length === 0) {
      ...
      yPos = newNode.y
        ? newNode.y -
          (ChartConsts.matchDistance.toPreviousMatch *
            ChartConsts.gridBaseSize) /
            2
  ...
  public getOutlierNeighbours(nodes: Node[]): IdType[] {
    let returned: IdType[] = []
    nodes.forEach((node) => {
      let neighborIds = this.chart.getNeighboursByEdge(
        node.id,
        (edge: Edge) => !ChartUtils.isFileEdge(edge)
      ).nodes
      neighborIds.forEach((neighbourId) => {
        let edgesOfNeighbourIds = this.chart.getNeighboursByEdge(
          neighbourId,
          (edge: Edge) => !ChartUtils.isFileEdge(edge)
        ).edges
  ...
  public deleteSelected() {
    ...
    let matchNodes: IdType[] = selection.nodes.filter((item) =>
      ChartUtils.isMatchNode(this.chart.getNode(item))
    )
    let newEdges: Edge[] = []
    matchNodes.forEach((nodeId) => {
      ...
      let connectedToMatchEdges = conncetedEdges
        .filter((i) => {
          return ChartUtils.isMatchEdge(i)
        })
        .filter((i) => i.to === nodeId)
      let edgesFromMatchToIds = conncetedEdges
        .filter((i) => {
          return ChartUtils.isMatchEdge(i)
        })
        .filter((i) => i.from === nodeId)
        .map((i) => i.to)
      connectedToMatchEdges.forEach((edge) => {
        edgesFromMatchToIds.forEach((toId) => {
          let newEdge = Utils.deepCopy(edge) as Edge
          newEdge.to = toId
          newEdge.id = newEdge.id
            .toString()
            .replace(nodeId.toString(), toId.toString())
          newEdges.push(newEdge)
        })
      })
    })
    let deletedFiles = this.chart
      .getItems(selection.nodes)
      .nodes.filter((i) => ChartUtils.isFileNode(i))
    this.app.removeFilesFromLegend(deletedFiles as FileNode[])
    this.chart.deleteItems(selection)

    let orphanedFiles = this.chart
      .getAllFileNodes()
      .filter((i) => !ChartUtils.isCustomNode(i))
      .filter(
        (i: FileNode) => this.getFileNodeMatcheNodes(i).length === 0 && i.hidden
      )
    this.app.removeFilesFromLegend(orphanedFiles as FileNode[])
    this.chart.deleteItems({
      nodes: orphanedFiles.map((i) => i.id),
      edges: [],
    })
  ...
  public getGroupBoundaryNodes(
    groupNodeId: IdType,
    includeFileNodeLogic
  ): VisiNode[] {
    return this.chart.getAllNodes(
      (i: VisiNode) =>
        i.d &&
        !i.hidden &&
        (i.id === groupNodeId ||
          (i.d.type === NodeTypes.boundaryNode &&
            i.d.belongsToGroup === groupNodeId) ||
          (includeFileNodeLogic && (i as MatchNode).d.ofFile === groupNodeId))
    ) as VisiNode[]
  }

  public extendSelection(selection: { nodes: IdType[]; edges: IdType[] }): {
    nodes: IdType[]
    edges: IdType[]
  } {
    console.log(
    ...
    let returnedSelection: { nodes: IdType[]; edges: IdType[] } =
      Utils.deepCopy(selection)
    // match nodes of file
    let fileNodes: IdType[] = selection.nodes.filter(
      (item: Node) =>
        !item.hidden && ChartUtils.isFileNode(this.chart.getNode(item))
    )
    ...
    let matchNodes: IdType[] = returnedSelection.nodes.filter((item) =>
      ChartUtils.isMatchNode(this.chart.getNode(item))
    )
    matchNodes.forEach((nodeId) => {
      let connected = this.getOutlierNeighbours(
        this.chart.getItems([nodeId]).nodes
      )
      connected = this.chart
        .getItems(connected)
        .nodes.filter(
          (node) =>
            ChartUtils.isDragWithParent(node as VisiNode) ||
    ...
    console.log(
      'selection after extension',
      returnedSelection.nodes.length,
      returnedSelection.nodes
    )
  ...
  isPathEdge(edge: Edge) {
    return (
      this.isPathNode(this.chart.getItem(edge.to) as Node) &&
  ...
  public getNodeContent(node): ContentOfMatch {
    ...
    let content = endLine
      ? this.app.currentFile.lines
          .slice(nodeLine, nodeLine + endLine)
          .join('\r\n')
  ...
  public setNodeTitle(node: Node, title) {
    ...
    if (ChartUtils.isFileNode(node))
      this.app.updateLabelInFileLegend(node as FileNode, title)
  }

  public getFileNodeMatcheNodes(
    fileNode: Node,
    includeFilenameNodes = true
  ): Node[] {
    ...
    matchNodes = matchNodes.concat(
      this.chart
        .getNeighboursByEdge(fileNode.id, (edge) => ChartUtils.isFileEdge(edge))
        .nodes.map((i) => this.chart.getNode(i))
    )
  ...
  public getSeletedFileMatchesRows(): { startRowNumber; endRowNumber }[] {
    ...
    return this.getFileNodeMatcheNodes(
      this.app.currentFile.node as Node,
      false
    ).map((i) => {
      return {
  ...
  groupUngroupFile(node: Node) {
    ...
    let fileNode = ChartUtils.isMatchNode(node)
      ? this.getFileNodeByPath((node as MatchNode).d.ofFile)
      : node
    if (!fileNode) {
    ...
    if (ChartUtils.getFileNodeIsGrouped(node)) {
      ...
      fileNode.y =
        ...
        (ChartConsts.matchDistance.toPreviousMatch * ChartConsts.gridBaseSize) /
          2
      fileNode.x =
        fileMatches.sort((a, b) => {
          return b.x - a.x
  ...
  reloadAllFileNodes(
    files: ReloadFilesResponse[],
    options: ReloadOptions = { markNullFiles: true }
  ) {
    options = Object.assign(
      { addFailedReloadToDiagram: true, markNullFiles: true },
      options
    )
    let newNodesAndItems: Array<Node | Edge> = []
    files.forEach((file) => {
      newNodesAndItems = newNodesAndItems.concat(
        this.reloadSingleFileNode(
          this.getFileNodeByPath(file.file) as FileNode,
          file,
          options
        )
      )
  ...
  reloadSingleFileNode(
    ...
    let addFailedReloadToReturned = (node: Node, originalLineText) => {
      ...
      let existingIndicators = this.chart.getNeighboursByEdge(node.id, (edge) =>
        ChartUtils.isFailedSyncIndicatorEdge(edge)
      )
      if (existingIndicators.nodes.length > 0) {
        ...
        returnedItems = returnedItems.concat(
          indicatorNode,
          existingIndicators.edges[0] as Edge
        )
      } else {
        let failed = CreateUtils.createFailedSyncNode(
          node as MatchNode,
          this.chart,
          originalLineText
        )
    ...
    let sortedMatchNodes: {
      node: Node
      startOffset
      endOffset
      contentOffset
    }[] = this.getFileNodeMatcheNodes(fileNode, false)
      .filter((i: MatchNode) => i.d.line !== '')
      .sort((a, b) => ChartUtils.getLineNumber(a) - ChartUtils.getLineNumber(b))
      .map((i: MatchNode) => {
        let j = Utils.deepCopy(i)
        j.d.endLineNumber = i.d.endLineNumber
          ? i.d.endLineNumber
          : i.d.lineNumber
        return {
          node: j,
    ...
    sortedMatchNodes.forEach((i) => {
      if (ChartUtils.getEndLineNumber(i.node)) sortedMatchNodesEndLines.push(i)
    })
    sortedMatchNodes.forEach((i) => {
      if (ChartUtils.getContentEndLine(i.node))
        sortedMatchNodesContentLines.push(i)
    })
    let startLineMatchNodeIndex = 0
    let endLineMatchNodeIndex = 0
    let contentLineMatchNodeIndex = 0
    let currentMatchStartLine = () =>
      ChartUtils.getLineNumber(sortedMatchNodes[startLineMatchNodeIndex].node)
    let currentMatchEndLine = () =>
      ChartUtils.getEndLineNumber(
        sortedMatchNodesEndLines[endLineMatchNodeIndex].node
      )
    let currentMatchContentLine = () =>
      ChartUtils.getContentEndLine(
        sortedMatchNodesContentLines[contentLineMatchNodeIndex].node
      )
    let lineOffset = 0
    ...
    diffAsArray.forEach((diffLine, index) => {
      ...
      if (
        startLineMatchNodeIndex < sortedMatchNodes.length &&
        indexInOriginalContent === currentMatchStartLine()
      ) {
        if (isDebug)
          console.log(
            `updated start offset from ${sortedMatchNodes[startLineMatchNodeIndex].startOffset} to ${lineOffset}`
          )
      ...
      if (
        endLineMatchNodeIndex < sortedMatchNodesEndLines.length &&
        indexInOriginalContent === currentMatchEndLine()
      ) {
        if (isDebug)
          console.log(
            `updated end offset from ${sortedMatchNodesEndLines[endLineMatchNodeIndex].endOffset} to ${lineOffset}`
          )
      ...
      if (
        contentLineMatchNodeIndex < sortedMatchNodesContentLines.length &&
        indexInOriginalContent === currentMatchContentLine()
      ) {
        if (isDebug) console.log('updated content offset')

        sortedMatchNodesContentLines[contentLineMatchNodeIndex].contentOffset =
          lineOffset
    ...
    let changedNodes = sortedMatchNodes.map((i) => {
      ...
      if (changedInfo.endContentLine)
        changedInfo.endContentLine += i.contentOffset
      return i.node
    })
  ...
  clearFailedReloadNodesIndicators() {
    let indicatorNodes = this.chart.getNodes(
      (i) => ChartUtils.isFailedSyncIndicator(i),
      'id'
    ) as IdType[]
    indicatorNodes.forEach((i) => {
      ...
      if (matchNodes.length > 0) {
        ...
        ChartUtils.setLine(
          this.chart.getItem(matchNodes[0]) as Node,
          this.chart.getItem(i)['d'].newLineText,
          this.chart
        )
      } else {
  ...
  getNodesInGroupBoundaries(
    groupNodeId: IdType,
    excludeSelf = true
  ): VisiNode[] {
    let boundaries = this.getGroupBoundaryNodes(groupNodeId, true).map((i) => ({
      x: i.x,
      y: i.y,
    }))
    const groupBoundary = this.chart.getBoundingBox(groupNodeId)
    let groupBoundaries = [
      { x: groupBoundary.left, y: groupBoundary.top },
      { x: groupBoundary.left, y: groupBoundary.bottom },
      { x: groupBoundary.right, y: groupBoundary.top },
      { x: groupBoundary.right, y: groupBoundary.bottom },
    ]
    if (boundaries.length === 0) return []

    const leftToRight = boundaries
      .concat(groupBoundaries)
      .map((i) => i.x)
      .sort((i, j) => i - j)
    const topToBottom = boundaries
      .concat(groupBoundaries)
      .map((i) => i.y)
      .sort((i, j) => i - j)
    ...
    const rect = {
      left: leftToRight[0],
      top: topToBottom[0],
      right: leftToRight[leftToRight.length - 1],
      bottom: topToBottom[topToBottom.length - 1],
    }

    let confinedNodes = this.chart.getAllNodes(
      (node) =>
        !node.hidden &&
        node.x >= rect.left &&
        node.x <= rect.right &&
    ...
    let confinedGroupNodes = confinedNodes.filter((i) =>
      ChartUtils.isGroupNode(i)
    )
    confinedGroupNodes.forEach((i) => {
      confinedNodes = confinedNodes.concat(
        this.getNodesInGroupBoundaries(i.id, false)
      )
    })

    // possibly include self in results
    if (!excludeSelf)
      confinedNodes.push(this.chart.getItem(groupNodeId) as VisiNode)
    return confinedNodes
  }

  collapseGroup(groupNodeId) {
    // un-attach all attached to group
    let existingGroupNodes = this.chart.getAllNodes(
      (i: VisiNode) => i.d && i.d.belongsToGroup === groupNodeId
    )
    this.chart.nodes.update(
      Utils.deepCopy(
        existingGroupNodes.map((i: VisiNode) => {
          if (i.d.type !== NodeTypes.boundaryNode)
    ...
    const toEdges = this.chart.getAllEdges(
      (i) =>
        groupNodeIds.indexOf(i.to) !== -1 && groupNodeIds.indexOf(i.from) === -1
    )
    const fromEdges = this.chart.getAllEdges(
      (i) =>
        groupNodeIds.indexOf(i.from) !== -1 && groupNodeIds.indexOf(i.to) === -1
    )
    const newEdges = toEdges
      .map((i) =>
        Utils.deepMerge(
          i,
          { to: groupNodeId, id: i.id + '_group' },
          { d: { type: EdgeTypes.collapseEdge } }
        )
      )
      .concat(
        fromEdges.map((i) =>
          Utils.deepMerge(
            i,
            { from: groupNodeId, id: i.id + '_group' },
            { d: { type: EdgeTypes.collapseEdge } }
          )
        )
      )
  ...
  expandGroup(groupNodeId) {
    let groupNodes = this.chart
      .getAllNodes((i: VisiNode) => i.d && i.d.belongsToGroup === groupNodeId)
      .map((i: VisiNode) => {
    ...
    let groupEdges = this.chart.getAllEdges(
      (i: VisiEdge) =>
    ...
    this.chart.setSelection({
      nodes: groupNodes.concat([groupNode]).map((i) => i.id),
      edges: [],
    })
  }
  ...
  getAttachedToGroup(groupNodeId) {
    return this.chart.getAllNodes(
      (i: VisiNode) => i.d.belongsToGroup === groupNodeId
    )
  }
}
