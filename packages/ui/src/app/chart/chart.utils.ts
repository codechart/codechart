...
export class ChartUtils {
  ...
  public static isFileNode(
    item: Node | Edge,
    excludeCustom: boolean = false
  ): boolean {
    if (!ChartUtils.isNode(item)) return false
    const isFileNode =
      ChartUtils.getMatchAttributes(item as Node) &&
      ChartUtils.getMatchAttributes(item as Node).fileContent
  ...
  public static setElementAttributesAndGet(element, newAttributes: any) {
    let newAttributesObject = Object.assign(
      {},
      element[AttributesKey],
      newAttributes
    )
  ...
  public static getSameMatch(
    ...
    let isSameEndline = (match1: MatchInfo, match2: MatchInfo) => {
      ...
      if (match1.lineNumber === match1.endLineNumber && !match2.endLineNumber)
        return true
      if (match2.lineNumber === match2.endLineNumber && !match1.endLineNumber)
        return true
      return false
    }
    try {
      let exisitingMatches = chart
        .getItems(chart.getAllItemIds().nodes)
  ...
  public static getMatchAttributes(
    element: Node
  ):
    | MatchInfo
    | FileNode
    | any /*so I dont need to cast result. sgould split this to get File and get Match atts*/ {
    return element[AttributesKey]
  }

  public static setAttributes(
    element: Node,
    newAttributes: MatchInfo | FileNode
  ) {
    element[AttributesKey] = newAttributes
  }

  public static getLineNumber(node: Node) {
    return ChartUtils.getMatchAttributes(node)
      ? ChartUtils.getMatchAttributes(node).lineNumber
      : null
  }

  public static getEndLineNumber(node: Node) {
    return (ChartUtils.getMatchAttributes(node) as MatchInfo)
      ? ChartUtils.getMatchAttributes(node).endLineNumber
      : null
  }
  ...
  static isMatchNode(node: Node): boolean {
    try {
      if (
        ...
        ChartUtils.getLineNumber(node) !== undefined &&
        ChartUtils.getLineNumber(node) !== null
      )
        return true
      else return false
    } catch (e) {
  ...
  static setContentEndLine(node: Node, lineNumber) {
    ;(ChartUtils.getMatchAttributes(node) as MatchInfo).endContentLine =
      lineNumber
  }
  ...
  public static getMiddlePoint = (
    ...
    return (
      nodes
        .map((i) => chart.getPosition(i.id)[xOrY])
  ...
  public static getMatchOfFilename(filenameNode: Node): IdType {
    return (filenameNode.id as String).substring(
      'filename_'.length,
      (filenameNode.id as String).length
    )
  }
  ...
  static getMatchCodeLineLabel(node) {
    let matchTrimmedLabel = () => {
      ...
      if (ChartUtils.isFailedSyncIndicator(node))
        return label.match(/.{1,30}/g).join('\n')

      if (label.length > ChartConsts.maxTitleLength)
        label = label.substring(0, ChartConsts.maxTitleLength) + '...'
      return label
    }
    ...
    let title =
      ChartUtils.getLineNumber(node) +
      (ChartUtils.getEndLineNumber(node)
        ? '-' + ChartUtils.getEndLineNumber(node)
        : '') +
      ':' +
      matchTrimmedLabel()
  ...
  static removeOrphanEdges(edges: Edge[], chart: ChartWrapper): Edge[] {
    return edges.filter((edge: Edge) => {
      {
        try {
          if (chart.getItem(edge.from) && chart.getItem(edge.to)) return true
          else {
  ...
  static isGroupNode(node: VisiNode) {
    return (
      ...
      (node.d.isCustom || node.d.type === NodeTypes.groupNode)
    )
  }
}
