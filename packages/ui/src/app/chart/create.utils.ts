...
export class CreateUtils {
  public static createOrUpdateMatchNode(
    match: MatchInfo,
    ofFileNodeId,
    chart: ChartWrapper,
    connectToNode: Node,
    ...
    if (matchNode === null) {
      matchNode = this.createMatchNode(
        match,
        ofFileNodeId,
        chart,
        additionalStyle
      ...
      if (!ChartUtils.isSameOfFileNode(matchAttributes.ofFile, ofFileNodeId)) {
        ...
        matchNode.y = null
        let fileEdge = chart
          .getItems(chart.getAllItemIds().edges)
          .edges.find((i) => {
            return (
              (i.from === match.id && i.to === matchAttributes.ofFile) ||
              (i.from === matchAttributes.ofFile && i.to === match.id)
            )
          })
        if (fileEdge) chart.deleteItems({ edges: [fileEdge.id], nodes: [] })
        else
          console.log(
            `no file edge found from match ${match.id} and file node ${matchAttributes.ofFile}`
          )
      }
    ...
    if (
      connectToNode !== null &&
      connectToNode.id !== matchNode.id &&
      !ChartUtils.isFileNode(connectToNode)
    ) {
      results.push(
        CreateUtils.createMatchEdge(
          chart,
          connectToNode.id,
          matchNode.id,
          matchNode.label
  ...
  public static createFileNameNode(
    fileName,
    node: Node,
    color: Color,
    chart: ChartWrapper
    ...
    let myInvert = invert
    let filenameNode = chart.createNode('filename_' + node.id, '', {
      d: { type: 'filename' },
    })
    ...
    filenameNode.color = {
      ...
      border: myInvert(color.background),
    }
    ...
    let filenameEdge = chart.createLink(
      node.id,
      filenameNode.id,
      CcItemStyles.filenameEdge,
      { idPrefix: 'filenameEdge' }
    )
  ...
  public static createFileEdge(chart: ChartWrapper, ofFileNodeId, matchNodeId) {
    return chart.createLink(ofFileNodeId, matchNodeId, CcItemStyles.fileLink, {
      idPrefix: 'fileEdge',
    })
  }

  public static createMatchEdge(
    chart: ChartWrapper,
    nodeToConnectId,
    matchNodId,
    matchValue
  ) {
    return chart.createLink(
      nodeToConnectId,
      matchNodId,
      CcItemStyles.matchMatchLink,
      { idPrefix: `match` }
    )
  }

  public static createFileNode(
    file: FindInFilesResponse,
    chart: ChartWrapper,
    existingFileColors: string[],
    xPos,
    folderInfo: CCPath
  ): FileNode {
    let pathChar = file.file.indexOf('\\') != -1 ? '\\' : '/'
    let fileName = file.file.substring(
      file.file.lastIndexOf(pathChar),
      file.file.length
    )
    let fileNode = chart.createNode(file.file, fileName, CcItemStyles.fileNode)
    fileNode.x = xPos
    ;(fileNode.color as Color).border = (
      Utils.getRandomColor_useList(existingFileColors) as NodeColor
    ).background
  ...
  public static createFailedSyncNode(
    node: MatchNode,
    chart,
    oldLineText
  ): { node: Node; edge: Edge } {
    // let failedNode = chart.createNode(, oldLineText)
    let failedNode = this.createMatchNode(
      {
        id: null,
        line: oldLineText,
        ofFile: ChartUtils.getOfFileId(node),
        lineNumber: node.d.lineNumber,
      },
      ChartUtils.getOfFileId(node),
      chart,
      CcItemStyles.failedSyncNode
    )
