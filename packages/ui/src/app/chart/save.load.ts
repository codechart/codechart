...
export class SaveLoad {
  ...
  public loadDataFromFindInFiles(response: FindInFilesResponse[]) {
    let matchCount = response.reduce((soFar, item) => {
      return soFar + item.matches.length
        ? /*matches in file*/ item.matches.length
        : /*file*/ 1
    }, 0)
    if (!this.ideConnect.getIsInIde())
      this.app.addMessage(
        'search results',
        'found ' + matchCount + ' matches in ' + response.length + ' files',
        2000
      )
    ...
    response.forEach((file: FindInFilesResponse) => {
      ...
      if (!fileNode) {
        fileNode = CreateUtils.createFileNode(
          file,
          this.chart,
          fileColors,
          this.app.selectedNode
            ? (this.app.selectedNode as Node).x - 300
      ...
      file.matches.forEach((match: MatchInfo) => {
        let matchNodes = CreateUtils.createOrUpdateMatchNode(
          match,
          fileNode.id,
          this.chart,
  ...
  public syncFiles(fileNodes: FileNode[], showMessage = true): Promise<any> {
    return new Promise((resolve, reject) => {
      interface PathsToFiles {
        [gitUrls: string]: { dirPath: string; filePaths: string[] }
      }

      // map available gitUrls to folder and filepaths array
      const pathsToFiles: PathsToFiles = fileNodes.reduce(
        (map: PathsToFiles, fileNode: FileNode) => {
          if (!map[fileNode.d.gitUrl]) {
            const projectPath = this.searchManagment.getPathByGitUrl(
              fileNode.d.gitUrl
            )
            ...
            map[fileNode.d.gitUrl].filePaths.push(
      ...
      for (let property in pathsToFiles) {
        ...
        reloadRequests.push(
          this.http.post(
            Env.getApiEndpoint() + EndPoints.reloadFiles,
            reloadBody
          )
        )
      }
      ...
      forkJoin(reloadRequests)
        ...
        .subscribe((responseList: any[]) => {
          ...
          const reloadedFiles: ReloadFilesResponse[] = responseList.reduce(
            (i: ReloadFilesResponse[], j: { files: ReloadFilesResponse[] }) => {
              return i.concat(j.files)
            },
            []
          )
          this.chartActions.reloadAllFileNodes(reloadedFiles, {
            markNullFiles: false,
          })
          if (showMessage)
            this.app.addMessage(
              `Finished synching ${this.app.syncPath}`,
              '',
              3000
            )
          resolve()
  ...
  public saveChartToJson(diagramData: QueryDto) {
    ...
    let jsonContent: DownloadInterface = {
      info: diagramData,
      nodes: savedData.nodes,
      edges: savedData.edges,
      dirPath: this.searchManagment.searchObject.searchPath,
      positioning: this.app.Options.positioning,
    }
  ...
  public prepareNodesAndEdgesForSave(): { nodes; edges } {
    ...
    let jsonSavedEdges = ChartUtils.removeOrphanEdges(
      this.chart.edges.get(),
      this.chart
    )
  ...
  public loadFromJson(jsonEvt) {
    ...
    // new format
    let loaded: { nodes; edges; dirPath; positioning } = {
      nodes: [],
      edges: [],
      dirPath: '',
      positioning: '',
    ...
    Utils.addIfNotExist(
      this.app.currentDiagramDetails.projectList,
      parsed.dirPath
    )
    // old format
  ...
  public saveToCode(files: { name; content }[]) {
    ...
    let filesReq: SaveToCodeRequest = {
      ...
      files: files.map((i) => {
        let filePath = i.name
        if (Utils.comparePaths(filePath, ccPath.folder) !== -1)
          filePath = filePath.substring(ccPath.folder.length)
        return { file: filePath, content: i.content }
      }),
    }
    this.http
      .post(Env.getApiEndpoint() + EndPoints.saveToCode, filesReq)
      .subscribe((response: { files: ReloadFilesResponse[] }) => {
        ...
        if (errorFiles.length === response.files.length) {
          ...
          if (reloadFiles.length > 0)
            message = `Succeded saving ${reloadFiles.length} files`
          if (errorFiles.length > 0)
            message += `; Failed saving ${errorFiles.length} files. ${errorFiles[0].error}`
        }
  ...
  public saveToCode2() {
    ...
    this.http
      .post(Env.getApiEndpoint() + EndPoints.saveToCode2, saveToFileJson)
      .subscribe((saveToFileResponse: SaveNodesResponse[]) => {
  ...
  public fullSaveToFile(filename) {
    ...
    let handleNodesIdsDifferentThanSavedIds = (
      response: SaveNodesResponse[]
    ) => {
      ...
      let resetIdsFuncPerhapsUseThis = (jsonResponse) => {
        jsonResponse.forEach((updatedId) => {
          ...
          let nodeEdges = this.chart
            .getItems(this.chartActions.getSurroundingEdgesIds(currentId))
            .edges.map((edge) => {
  ...
  public load(loaded: { nodes: Node[]; edges: Edge[] }) {
    if (!this.app.Options.keepChartOnLoadFromJson)
    ...
    loaded.nodes = loaded.nodes.map((node: Node) => {
      try {
        if (ChartUtils.isMatchNode(node)) {
          let sameNode = ChartUtils.getSameMatch(
            this.chart,
            ChartUtils.getMatchAttributes(node),
            ChartUtils.getOfFileId(node)
          )
          if (sameNode) {
            loaded.edges.push(
              this.chart.createLink(
                node.id,
                sameNode.id,
                CcItemStyles.suspectedSameMatchLink,
                { idPrefix: 'sameMatch' }
              )
            )
          }
    ...
    this.chart.simpleLoadFromJson(loaded, {
      fitToAll: true,
      selectLoaded:
        this.chart.nodes.length > 0 && this.app.Options.keepChartOnLoadFromJson,
  ...
  private createSaveToCodeSentData(): SaveJson {
    let savedNodes: SaveNode[] = this.chart.nodes.get().map((node: Node) => {
      return CreateTypes.createSaveNode(
        ChartUtils.getLineNumber(node) as number,
        ChartUtils.getOfFileId(node),
        node.id as string
      )
    })
    return {
      nodes: savedNodes,
      dirPath: this.searchManagment.searchObject.folderPath.folder,
    }
  }
