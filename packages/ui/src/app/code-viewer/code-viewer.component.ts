...
export class CodeViewerComponent implements OnInit {
  ...
  @Input() set fileData(fileData: CurrentFile) {
    ...
    if (!fileData) {
      this._fileData = null
      session.setValue(
        'No file selected, so nothing to display. Select a node on the chart to the right.....'
      )
    ...
    if (this.fileData) {
      this.sessionInfos[this.fileData.name] = {
        folds: this.editor.getEditor().getSession().getAllFolds(),
      }
    }
    ...
    this.fileDisplayInfo = {
      folder: this._fileData.isCustom
        ? ''
        : this._fileData.name.replace(/^.*[\\\/]/, ''),
      file: this._fileData.isCustom
        ? this._fileData.name
        : this._fileData.name.replace(/\w+\..*/, ''),
      color:
        this.fileData.node && (this.fileData.node.color as Color).border
          ? (this.fileData.node.color as Color).border
          : 'black',
    }
  }
  ...
  setMode() {
    ...
    if (
      this.fileData.name &&
      (!this.fileData.name.split('.').length ||
        this.fileData.name.split('.').length === 1)
    ) {
    ...
    let selectedLanguage = languages.get(suffix)
      ? languages.get(suffix)
      : languages.get('text')
  ...
  ngOnInit() {
    ...
    this.aceEditor.getSelection().on('changeCursor', (a, b, c) => {
      ...
      if (selection.getAnchor().row == 0 && selection.getAnchor().column == 0)
        return
    ...
    this.aceEditor.on('blur', (event) => {
      this.blurEvent(event)
    })
    this.aceEditor.on('focus', (event) => {
  ...
  changeText(event) {
    this.textChangedEvent.emit({
      delta: event,
      text: this.aceEditor.session.getValue(),
    })
  }
  ...
  public markLinesSelected(startRowNumber, endRowNumber) {
    ...
    this.lastAddedMarker = this.aceEditor
      .getSession()
      .addMarker(range, 'marker', 'fullLine')
  }

  setRangeForStartEndLines(
    range: Ace.Range,
    startRowNumber,
    endRowNumber
  ): Ace.Range {
  ...
  public markMatchesInFile(matches: { startRowNumber; endRowNumber }[]) {
    ...
    matches.forEach((i) => {
      ...
      let addedMarker = this.aceEditor
        .getSession()
        .addMarker(range, 'matchMarker', 'fullLine')
  ...
  saveFile() {
    this.appComponent.saveLoad.saveToCode([
      {
        name: this.appComponent.currentFile.name,
        content: this.aceEditor.session.getValue(),
      },
    ])
  }
}
