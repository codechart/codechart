...
export class NodeStylingComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    if (
      this.chart.getSelection().nodes.length > 0 ||
      this.chart.getSelection().edges.length > 0
    )
  ...
  public applyManualStyle() {
    ...
    this.chartActions.setSelectionStyle(
      JSON.parse(this.styleElement.nativeElement.value)
    )
    this.chart.refresh()
  }
  ...
  public setSelectionNodeStyle(color: NodeColor) {
    this.chart.setColor(
      this.chartActions.getSelectedLinksOrNodesOnly(),
      color.background
    )
    this.chart.setBorderColor(
      this.chartActions.getSelectedLinksOrNodesOnly(),
      color.background,
      true
    )
  ...
  public setSelectionImage(imagePath) {
    this.chart.setNodeImage(
      this.chartActions.getSelectedLinksOrNodesOnly().nodes,
      imagePath
    )
  }

  public setSelectionShape(shape) {
    this.chart.setNodeShape(
      this.chartActions.getSelectedLinksOrNodesOnly().nodes,
      shape
    )
  }
  ...
  public setTitle(event: Event) {
    ...
    this.chartActions.setNodeTitle(
      this.selectedNode as Node,
