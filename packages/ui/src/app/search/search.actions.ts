...
export class SearchActions {
  ...
  public searchSelectedFile() {
    ...
    this.doSearch(
      Object.assign({}, this.searchManagement.searchObject, {
        searchPath: ChartUtils.getFilePath(fileNode),
      }),
      SearchEnum.searchInFile
    )
  }
  ...
  public contentSearch() {
    ...
    let regex = new RegExp(
      ...
      this.searchManagement.searchObject.flags
    )
    contentLines.forEach((line, index) => {
      ...
      let matchInfo: MatchInfo = {
        ...
        indexInLine: 0,
        id: CreateUtils.createId(
          ChartUtils.getOfFileId(this.app.selectedNode as Node),
          lineNumber
        ),
  ...
  public async doSearch(
    ...
    if (!searchJson.folderPath || searchJson.folderPath === {}) {
      this.app.addMessage(
        'no path defined',
        'no path defined, try selecting another path then reselect current path ',
        5000
      )
      return
    }
    let searchRequest = Object.assign({}, searchJson, {
      searchType: searchType,
    })
    await this.http
      .post(Env.getApiEndpoint() + EndPoints.find, searchRequest)
      .pipe(
        map((i: FindInFilesResponse[]) =>
          i.map((i) =>
            Object.assign(i, {
              gitUrl: this.searchManagement.searchObject.folderPath.gitUrl,
            })
          )
        )
      )
      ...
      .then((response: FindInFilesResponse[]) => {
        ...
        if (!response.length) {
          this.app.addMessage(
            'No results found',
            'no results found in folder ' + searchJson.folderPath.folder,
            -1
          )
          return
        }
        ...
        this.chart.getAllFileNodes().forEach((fileNode: FileNode) => {
          let correspondingFile = response.find((responseFile) => {
            const projectFolder = searchJson.folderPath.folder.replace(
              /[/\\]/g,
              ''
            )
          ...
          if (
            correspondingFile &&
            correspondingFile.content !== fileNode.d.fileContent
          ) {
        ...
        if (!areFilesSynched) {
          this.app.addMessage(
            ...
            'Seems that some of the files on disk are not identical to those in diagram. ' +
        ...
        if (matchCount < Options.minResultsCountToShowResults)
          this.loadResults(response, null, true)
  ...
  public displaySearchResults(results: FindInFilesResponse[], callback) {
    ...
    setTimeout(() => {
      this.saveLoad.loadDataFromFindInFiles(results)
    }, 300)
  ...
  public createMatchFromSelection(
    ...
    let selection: AceSelectionRange =
      this.app.codeEditor.aceEditor.getSelectionRange()
    ...
    if (selectedNode === null) {
      this.app.addMessage(
        'must select node',
        'can`t create selected node without first selecting node',
        3000
      )
      return null
    }
    ...
    let endLineNumber =
      selection.end.row !== selection.start.row ? selection.end.row : null

    let fileFullPath =
      codeEditor.fileDisplayInfo.folder + '//' + codeEditor.fileDisplayInfo.file
    let matchId: string = !replaceSelected
      ? CreateUtils.createId(fileFullPath, startLineCounter)
      : selectedNode.id.toString()
    let endContentLine = Utils.getEndLineOfBlock(
      codeEditor.fileData.lines,
      startLineCounter
    )
    let match: MatchInfo = {
      line: startLineText,
    ...
    if (!replaceSelected) {
      ...
      let matchNode = CreateUtils.createMatchNode(
        match,
        ofFileNodeId,
        this.chart
      )
