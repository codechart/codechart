...
export class SaveLoadService {
  getById(id: any): Observable<ResultDiagramUI> {
    return this.http
      .get(Env.getApiEndpoint() + EndPoints.loadDiagram + id)
      .pipe(
        map((response: FullDiagramDto) => {
          let result: ResultDiagramUI = Object.assign(response, {
  ...
  save(params: SaveInfo, isNew = true) {
    let savedInfo: CreateDiagramDto = Object.assign(
      ...
      params.savedDiagramDetails,
      {
        labels: params.labels,
        fileNames: params.filenames,
        projects: params.savedDiagramDetails.projectList,
      }
    )
    ...
    if (isNew) {
      ...
      return this.http.post(
        Env.getApiEndpoint() + EndPoints.createDiargam,
        savedInfo
      )
    } else
      return this.http.post(
        Env.getApiEndpoint() + EndPoints.updateDiagram,
        savedInfo
      )
  }
  ...
  public getResults(searchObject: QueryDto): Promise<ResultDiagramUI[]> {
    return this.http
      .post(Env.getApiEndpoint() + EndPoints.searchDiagram, searchObject)
      .pipe(
  ...
  deleteDiagram(id) {
    return this.http.post(
      Env.getApiEndpoint() + EndPoints.deleteDiagram + id,
      {}
    )
  }
}
